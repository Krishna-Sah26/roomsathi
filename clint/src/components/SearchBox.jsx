  ...
  return (
    <div className={isDark ? "mx-auto w-full max-w-4xl rounded-2xl border-4 border-blue-500 bg-slate-950 p-3 shadow-[0_20px_45px_rgba(2,6,23,0.35)]" : "mx-auto w-full max-w-4xl rounded-2xl border-4 border-blue-700 bg-white p-3 shadow-[0_20px_45px_rgba(15,23,42,0.18)]"}>
      <div className="grid grid-cols-1 gap-3 md:grid-cols-[1.3fr_1fr_1fr_auto]">
        <label className={isDark ? "flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-slate-300" : "flex items-center gap-3 rounded-xl px-4 py-3 text-sm text-slate-500"}>
          ...
          <input
            ...
            className={isDark ? "w-full bg-transparent text-sm text-white outline-none placeholder:text-slate-400" : "w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400"}
          />
        </label>

        <label className={isDark ? "flex items-center justify-between gap-3 rounded-xl px-4 py-3 text-sm text-slate-300" : "flex items-center justify-between gap-3 rounded-xl px-4 py-3 text-sm text-slate-500"}>
          <div className="flex items-center gap-3">
            <Home className="shrink-0 text-blue-600" size={18} />
            <select
              value={roomType}
              onChange={(e) => setRoomType(e.target.value)}
              className={isDark ? "w-full bg-slate-950 text-sm text-white outline-none" : "w-full bg-transparent text-sm text-slate-900 outline-none"}
            >
        ...
        <button
          ...
          className="rounded-xl bg-blue-700 px-7 py-3 text-sm font-semibold text-white transition hover:bg-blue-800"
        >
          Search
        </button>
      </div>
